...
import { useGetAllEntries, useDeleteEntry, useIsCallerAdmin } from '../hooks/useQueries';
...
import { formatAuthorLabel, formatTimestamp, decodeComment, decodePlaceNames } from '../lib/guestbookFormat';
import { Loader2, BookOpen, Pencil, Trash2, MapPin } from 'lucide-react';
  ...
  if (sortedEntries.length === 0) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <p className="text-muted-foreground mb-6">Be the first to sign the VTH Guest Book!</p>
        <Button onClick={() => navigate({ to: '/add' })}>Sign the Guestbook</Button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
      ...
      {sortedEntries.map((entry) => {
        ...
        const preview = cleanComment.length > 160 ? cleanComment.slice(0, 160) + '…' : cleanComment;
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div
                ...
                onClick={() =>
                  navigate({ to: '/entry/$timestamp', params: { timestamp: entry.timestamp.toString() } })
                }
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {entry.trailName && (
                    <span className="text-xs text-primary font-medium">🥾 {entry.trailName}</span>
                  )}
                </div>
                <p className="text-sm text-muted-foreground leading-relaxed mb-2">{preview}</p>
