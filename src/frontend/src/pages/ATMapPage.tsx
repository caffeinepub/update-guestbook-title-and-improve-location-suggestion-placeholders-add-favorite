...
export default function ATMapPage() {
  const { data: entries = [], isLoading } = useGetEntriesWithFavoritePlace();
  ...
  useEffect(() => {
    if (!mapReady || !mapInstanceRef.current || typeof L === "undefined")
      return;
    ...
    const pinIcon = L.divIcon({
      ...
      html: `<svg width="28" height="36" viewBox="0 0 28 36" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <filter id="shadow-at" x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="0" dy="2" stdDeviation="2" flood-color="rgba(0,0,0,0.35)"/>
          </filter>
        </defs>
        <path d="M14 0C6.268 0 0 6.268 0 14c0 9.333 14 22 14 22S28 23.333 28 14C28 6.268 21.732 0 14 0z" fill="#8B4513" filter="url(#shadow-at)"/>
    ...
    for (const entry of validEntries) {
      const lat = entry.favoritePlace!.latitude;
      ...
      const popupContent = `
        <div style="font-family:sans-serif;min-width:140px;">
          ...
          <a href="/entry/${timestamp}" style="color:#8B4513;font-size:12px;font-weight:600;cursor:pointer;text-decoration:underline;margin-top:6px;display:block;" onclick="event.preventDefault(); window.history.pushState(null,'','/entry/${timestamp}'); window.dispatchEvent(new PopStateEvent('popstate'));">View Entry →</a>
        </div>
      `;
    ...
    <div className="flex flex-col h-full bg-background">
      <div className="px-4 pt-4 pb-2">
        ...
        <p className="text-sm text-muted-foreground">
          Favorite places along the trail
        </p>
      </div>
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
              <div className="bg-card/90 rounded-lg px-4 py-3 text-center shadow">
                ...
                <p className="text-muted-foreground text-xs mt-1">
                  Add an entry with your favorite trail place!
                </p>
              </div>
            </div>
          )}
